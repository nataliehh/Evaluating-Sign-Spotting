...
from tools.tools import store_dict
...
'''
def process_video(video_path:str, cngt_vid:bool, draw:bool = False, only_face:bool = False):
...
'''        
def save_landmarks_for_video_path(path:str, file:str, new_path:str, log:bool = True, draw:bool = True):
    ...
    landmarks = process_video(path+file, cngt_vid, draw = draw)
    ...
    for lmrk_key in landmarks:
        np.save(new_path+file_no_extension+'_'+lmrk_key, landmarks[lmrk_key])
    if log: print('Done.')
    return landmarks
