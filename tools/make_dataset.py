...
def make_dataset(anns_with_tiers, features, df, val_vids, test_vids, top_signs, id_split, fixed_length = 10, zero_pad = False, only_features = True):
    # Creates empty list for each of these
    ...
    for i, k in enumerate(anns_with_tiers):
        ...
        if train_element:
            ...
            if only_features:
                # Wrist (x coord = ind 45) + fingertips (x coord = every other ind in 61...70)
                mirror_ind = [45] + list(range(61,71,2))
        ...
        for item in items:
            ...
            if l.shape[0] == 0 or r.shape[0] == 0:
                ...
                # This is the case if no landmarks are detected
                lmrks = np.append(l, r, axis = 1)
                num_lmrks = lmrks.shape[0]
                lmrks_present = np.arange(num_lmrks) #np.unique(np.where(~np.isnan(lmrks))[0]) 
                lmrks = lmrks[lmrks_present]
                ...
                if train_element:
                    ...
                    mirror_lmrks = mirror_lmrks[lmrks_present]

                # This stores the landmark features *after* removing empty frames
                new_num_lmrks = lmrks.shape[0]

                # If at least some the frames have detected landmarks, we continue processing
                if new_num_lmrks/num_lmrks > 0.0: 
                    ann_lengths.append(new_num_lmrks)
    ...
    result = ([X_train, X_train_no_mirr, X_val, X_test], [y_train, y_train_no_mirr, y_val, y_test], 
              [smaller, precise, larger], ann_lengths, glosses)
    return result