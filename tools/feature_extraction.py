...
def compute_wrist_velocity(wrist, t1 = None, t2 = None, remove_nan = False):
    ...
    wrist_vel = []
    for frame in range(wrist.shape[0]-1):
        # Compute the left wrist distance for this frame and the next frame
        x1, x2 = wrist[frame, 0], wrist[frame+1, 0]
        y1, y2 = wrist[frame, 1], wrist[frame+1, 1]
        wrist_vel.append(get_distance(x1, y1, x2, y2))
    return wrist_vel # return velocity averaged by number of frames
...
def calc_hand_distance(hand_data, selection = True):
    ...
    if selection:
        hand_selection = [0, 4, 5, 8, 9, 12, 13, 16, 17, 20]
        hand_data = hand_data[hand_selection, :]
    
    # Computes sqrt((x1-x2)**2 + (y1-y2)**2) for all point pairs, result is N x N matrix 
    # We only want what's above the diagonal bc it's symmetrical and diagonal sim. is self-similarity (=1)
...
def calc_distance_per_frame(hand_data):
    ...
    distance_per_frame = []
    for frame in range(hand_data.shape[0]):
        d = calc_hand_distance(hand_data[frame])
        distance_per_frame.append(d)
...
def get_wrist_angle_distance(lmrks, key, pose = None):
    ...
    if pose is not None:
        ...
        for f in range(fingertips.shape[1]):
            finger = fingertips[:, f, :]
            # Get the index (pinky, index, or thumb) indices from the Hand model of Mediapipe
            lmrk_equivalent = [20,8,4][f]
    ...
    # Putting them together for each frame as (distances, wrist, angles)
    features = np.append(dist, wrist, axis = 1)
    features = np.append(features, angle, axis = 1)
    ...
    # We add 0 as the velocity of the first frame 
    velocity = np.array([0] + compute_wrist_velocity(wrist)).reshape(-1,1)
